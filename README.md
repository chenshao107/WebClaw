...
既然方案已经明确，我建议我们下一步直接进入 **“心脏”部分** 的开发：

**实现一个 `CodeInterpreter` 类。**
这个类需要具备：

1. **持久化：** 能保持 Playwright 的 `browser` 和 `page` 对象在多个代码块执行间不销毁。
2. **安全性：** 限制执行环境，捕获所有 `stdout` 和 `stderr`。
3. **反馈感：** 将执行过程中的“观察结果”精准地喂给执行层 Agent。
